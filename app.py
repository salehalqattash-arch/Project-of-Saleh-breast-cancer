...
"""
Flask API - Tumor Detection System (SAFE VERSION)
"""
...
from models.tumor_model import (
    get_system, reset_system,
    FEATURE_NAMES, FEATURE_LABELS_AR, FEATURE_RANGES,
    load_csv_dataset, TumorDetectionSystem
)
...
def get_image_detector():
    ...
    if not TF_AVAILABLE:
        print(" TensorFlow disabled → image model unavailable")
        return None
    ...
    if _image_detector is None:
        try:
            ...
            if os.path.exists(model_path):
                ...
                print(" No trained image model found")
...
def predict_image():
    ...
    try:
        if not TF_AVAILABLE:
            return jsonify({
                "success": False,
                "error": "TensorFlow not available on this system"
            }), 500
...
if __name__ == '__main__':
    print("=" * 50)
    print(" Tumor Detection System ")
    print("=" * 50)

    s = get_system()
    print(f"{s.data_stats}")
    print(f" Best model: {s.best_model_name}")

    if TF_AVAILABLE:
        print(" Image model supported")
    else:
